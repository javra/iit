...
def collectSortNames (its : Array InductiveType) : Array Name :=
its.map InductiveType.name

def isSortApp (e : Expr) (its : Array InductiveType) : Bool :=
match e with
...
def addEIfSort (n : Name) (its : Array InductiveType) : Name :=
if contains (collectSortNames its) n then n ++ erasureSuffix
else n

def eraseCtor (e : Expr) (its : Array InductiveType) : Expr :=
match e with
...
| fvar n d        => mkFVar (addEIfSort n its)
| _               => e

def erase (its : Array InductiveType) : Array InductiveType :=
